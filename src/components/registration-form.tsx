...
const RegistrationForm = ({
  ...
  const handleSubmit = async () => {
    ...
    const nameValidation = emailOnly
      ? { ok: true as const, sanitized: "", first_name: "", last_name: "" }
      : validateName(name);
    if (!nameValidation.ok) {
      capture("registration_form_validation_failed", {
        reason: "name",
    ...
    const phoneValidation = emailOnly
      ? { ok: true as const, sanitized: "" }
      : validatePhone(phone);
    ...
    try {
      ...
      setError(
        emailOnly
          ...
          : "We could not save your seat right now. Please try again.",
      );
    } finally {
      setLoading(false);
    }
            ...
            {emailOnly ? (
              <>
                <div className="text-[25px] leading-8">
                  Notify Me <span className="text-primary">When It Launches</span>
                </div>
                <div className="text-[20px] opacity-80">
                  — just your email. We&apos;ll let you know the moment
                  it&apos;s live
                </div>
              </>
            ) : (
          ...
          >
            ...
            <div
              className={`
                ...
                overflow-hidden
                ${show_phone_input && !emailOnly ? "opacity-100 translate-y-0" : "max-h-0 opacity-0 translate-y-2"}
              `}
            >
              <Field className="flex flex-col gap-y-0.5">
                <div className="select-none pl-5 text-sm tracking-[-0.8px] text-secondary/60">
